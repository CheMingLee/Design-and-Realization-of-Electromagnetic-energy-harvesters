import numpy as np
from numpy import pi, sin, cos
import pandas as pd
import matplotlib.pyplot as plt
import os

rg = 5
ke = 0.028
Ri = 2.7

Re_list = [1.5, 3.3, 6.8, 10.0, 15.0, 30.0, 51.0]
T_list = np.array([1.2359, 0.9838, 0.6998, 0.6014, 0.5442, 0.4734, 0.4412])
w_list = 2*pi/T_list

for Re, w in zip(Re_list, w_list):
    path_Re = '{0}ohm'.format(Re)
    data_V = os.path.join(path_Re, 'scope_2.csv')

    df = pd.read_csv(data_V, header=1)
    t_exp = df['second']
    V_exp = df['Volt']

    time = np.arange(0, 2+1e-4, 1e-4)
    R = Re+Ri
    W_m = w*rg
    W_e = W_m*3*8
    i_phase = ke*W_m*cos(W_e*time)/R

    plt.figure()
    # plt.title('Re={0} (ohm)'.format(Re))
    plt.xlabel('Time (s)', fontsize=14)
    plt.ylabel('Power (W)', fontsize=14)
    plt.xticks(fontsize=14)
    plt.yticks(fontsize=14)
    plt.grid()
    plt.plot(time, i_phase**2*Re, 'C1', label='Sim.')
    plt.plot(t_exp, V_exp**2/Re, 'C0', label='Exp.')
    plt.xlim([0, 2])
    plt.legend(loc=1, fontsize=14)
    plt.savefig('./fig/{0}ohm.png'.format(Re), bbox_inches='tight')
