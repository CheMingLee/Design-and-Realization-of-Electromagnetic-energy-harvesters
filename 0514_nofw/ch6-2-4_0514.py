...
for feq in feq_list:
    for amp in amp_list:
        ...
        plt.title('{0}Hz, {1}mm'.format(feq, amp), fontsize=14)
        plt.xlabel(r'External electrical load ($\Omega$)', fontsize=14)
        plt.ylabel('Energy harvester damping (Ns/m)', fontsize=14)
        plt.xticks(x, Re_list, fontsize=14)
        plt.yticks(fontsize=14)
        # plt.ylim([800, 3000])
        plt.legend(['Sim.', 'Exp.'], loc=1, fontsize=14)
        plt.grid()
        plt.savefig('./ch6-2-4/Ceq_{0}hz{1}mm.png'.format(feq, amp), bbox_inches='tight')
